...
class LineDetector:
	def __init__(self):
		self.conf_file = os.path.dirname(os.path.abspath(__file__)) + "/default.yaml"
	...
	def detect_cnt(self,image,color_hsv):
		...
		if len(color_hsv) == 1:
			color = color_hsv[0]
			mask = cv2.inRange(hsv, np.array(color['min']), np.array(color['max']))
		elif len(color_hsv) >= 2:
			for color in color_hsv:
				maski = cv2.inRange(hsv, np.array(color['min']), np.array(color['max']))
				if mask is None:
					mask = maski
				else:
