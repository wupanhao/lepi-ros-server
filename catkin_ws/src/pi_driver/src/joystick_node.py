...
class JoystickNode:
	def __init__(self):
		...
		self.joy = MyJoy(callback = self.cbPublishJoyState)
		self.json_state = json.dumps(self.joy.getState())
		self.srv_get_button_value = rospy.Service('~get_button_value', GetInt32, self.cbGetButtonValue)
		self.srv_get_axis_value = rospy.Service('~motor_get_position', GetInt32, self.cbGetAxisValue)
		self.srv_get_joy_state =rospy.Service("~get_joy_state", GetString, self.cbGetJoyState)
