...
class PupperDriverNode:
    ...
    def cbSetCommand(self, params):
        try:
            ...
            if 'roll' in data:
                self.command.roll = limit_to(data['roll'],self.config.max_roll)
            if 'pitch' in data:
                self.command.pitch = limit_to(data['pitch'], self.config.max_pitch)
            if 'yaw_rate' in data:
                self.command.yaw_rate = limit_to(data['yaw_rate'], self.config.max_yaw_rate)
            if 'height' in data:
                if data['height'] < 0.08:
                    data['height'] = 0.08
                if data['height'] > 0.22:
